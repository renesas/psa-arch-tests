...
int nvmem_ra6m4_write(addr_t base, uint32_t offset, void *buffer, int size)
{
    ...
    err = (int)FSP_GUARD_g_flash0_R_FLASH_HP_BlankCheck((base+offset),64, &blank_check_result);

    /* Validate the blank check result */
    if (FLASH_RESULT_NOT_BLANK == blank_check_result)
    {
        /* Erase Block */
        err = (int)FSP_GUARD_g_flash0_R_FLASH_HP_Erase((base+offset), 1);
        err = (int)(FSP_GUARD_g_flash0_R_FLASH_HP_Write((uint32_t)buffer, (base+offset), (uint32_t)size));
    }
